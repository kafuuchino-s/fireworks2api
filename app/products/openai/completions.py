...
async def completions(request: Request):
    ...
    try:
        payload, _, report = build_completions_adapter(context)
    ...
    upstream_path = FIREWORKS_INFERENCE_PATHS["completions"]
    route_trace = build_route_transform_trace(
        context,
        public_route="POST /v1/completions",
        operation="create",
        adapter="app.products.openai.completions.build_completions_adapter",
        fireworks_endpoint="completions:completions",
    ...
    record_proxy_transform_debug(context, endpoint="completions", upstream_endpoint=upstream_path, payload=payload, headers=headers, stream=bool(payload.get("stream")), service_tier=payload.get("service_tier") if isinstance(payload.get("service_tier"), str) else None, field_changes=report["field_changes"], warnings=report["warnings"])
